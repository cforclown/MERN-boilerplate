...
import App from '../../app';
import { mockExplorationPayload, mockSchedule, mockUpdateSchedulePayload } from '../../test/mock-data';
...
describe('schedules-router', () => {
        ...
        exploration: {
          ...
          pagination: {
            ...
            sort: {
              ...
              order: mockExplorationPayload.pagination.sort.order ?? 1
            }
          }
        }
      });
  ...
  describe('delete', () => {
    it('should successfully delete a schedule', async () => {
      ...
      const body = JSON.parse(response.text);
      expect(body).toHaveProperty('data');
      expect(body.data).toEqual(mockSchedule._id);
    });
